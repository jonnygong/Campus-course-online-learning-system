<%@ WebHandler Language="C#" Class="load_course_list_recent" %>
...
public class load_course_list_recent : IHttpHandler {
    ...
    {
        //执行查询操作，并获得返回的行数
        DataSet ds = DBHelper.sqlMethod("select major_id,course_img_url,course_name,major_name,teacher_name,course_visit from t_course,t_major,t_teacher,t_course_team where t_course.course_profession=t_major.major_id and t_course_team.teacher_id=t_teacher.teacher_id  and t_course_team.course_id = t_course.course_id and t_course_team.is_leader = 1 order by t_course.course_updateTime desc");//and major_id=" + id + " 
